...
test('greeting has small slow head turns, a smiling bow and exact rest', () => {
  ...
  for(let t=0;t<3.6;t+=.01) {
    assert.ok(Math.abs(greetingPose(t).turn)<.053);
    assert.ok(Math.abs(greetingPose(t+.01).turn-greetingPose(t).turn)/.01<.19);
  }
...
test('head rotation foreshortens eye spacing without affecting arms or tail', () => {
  ...
  for(let t=0;t<3.6;t+=.04) {
    ...
    for(const point of [[.281,.722],[.726,.719],[.41,.90],[.61,.90],[.755,.465]])
...
test('pose position and velocity remain continuous across all phase changes', () => {
  const boundaries=[.15,.23,.38,.45,.58,.8,.95,1.55,1.65,1.75,1.95,1.99,2.04,2.18,2.22,2.33,2.4,2.63,2.65,2.8,2.82,3.6];
...
test('reduced motion keeps the smile with smaller head movement and a gentler blink', () => {
  for(const t of [.62,1.13,2.25,2.4]) {
    ...
    for(const key of ['roll','turn','nod','tilt','squash'])assert.ok(Math.abs(calm[key])<=Math.abs(normal[key])*.201);
...
test('aegyo moves the head while both shoulders and torso stay still', () => {
  ...
  assert.ok(largestHeadMotion>.002&&largestHeadMotion<.01);
});
...
test('subtle yaw preserves eye proportions and leaves the head outline intact', () => {
  ...
  for(const [time,direction] of [[.95,1],[1.95,-1]]) {
    ...
    assert.ok(Math.abs(nose[0]-.506)<.01);
