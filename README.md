...
- **머리 쓰다듬기**: 이마 위를 누른 채 천천히 쓸면 눈을 살짝 감고 고개를 기울이며 꼬리를 살랑거립니다. 인식 후에는 누름과 늘림을 줄여 부드럽게 반응합니다.
- **배 간지럽히기**: 배를 짧게 좌우로 두세 번 문지르면 눈웃음과 작은 몸 떨림, 분홍 혀가 보이는 웃음으로 반응합니다. 한 방향으로 당기거나 빠르게 드래그하면 기존 늘리기를 유지합니다.
- **놀이 안내**: 상단 `?` 버튼에서 손길, 동작 버튼, 졸기와 깨기를 소개하는 3단계 모달을 엽니다. 안내 중에는 놀이와 감각 피드백을 멈추며, 닫으면 다시 시작합니다. 하단 버튼의 수와 높이는 유지합니다.
...
누름·당김·도약·인사 모두 얼굴과 몸은 원본 텍스처를 사용합니다. 인사는 팔 분리나 배경 복원 없이 연속된 메시를 변형합니다. `greeting-motion.js`에서 고개·입꼬리·눈매와 별도 투명 레이어의 볼 홍조를 제어합니다. 애교의 미소가 깊어질 때 원래 W자 입 아래에 작은 입 안과 분홍 혀를 합성하며, 얼굴의 변형 좌표를 따라갑니다. `touch-reactions.js`는 터치 부위·이동 거리·속도·방향 전환으로 쓰다듬기와 간지럽히기를 구분하고, 새 반응과 깨는 인사의 표정을 계산합니다. 원본 이미지는 변경하지 않습니다. `character-texture.js`가 원본에서 바깥과 연결된 회색 배경 및 발밑 그림자를 렌더링 시 투명하게 처리합니다. 눈과 입은 보존하며, WebGL과 Canvas 2D에 동일한 투명 텍스처를 사용합니다. 탭 숨김·포커스 이탈·모달 열기에는 놀이와 감각 피드백을 중지합니다.
